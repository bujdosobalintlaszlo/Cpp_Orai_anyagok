...
// ============================================================================
// SESSION 12 DATA STRUCTURES START — game + 19_SortedList + 20_PriorityStack
//
// Baseline: sessions 1–10 in this file; stl_game.h = completed quest + algorithms
// (same contract as session 11 tests).
//
// Student / live session work: sorted_list.h and priority_stack.h (TODO starters).
//
// Game integration:
//   - SortedList<std::string> — treasure names and active quest titles (sorted order)
//   - priority_stack<string,int> — loot “presentation order” (quest/gear first)
// ============================================================================
...
class Item {
   ...
   public:
    ...
    struct MatchesName {
        ...
        MatchesName(const std::string& name) : name(name) {}
        bool operator()(const Item& item) { return item.name_ == name; }
    };
};
...
class Inventory {
   private:
    typedef std::array<Item, Cap> container_t;
    container_t items_;
   ...
   public:
    ...
    bool addItem(const std::string& name, int value) {
        ...
        items_[size] = Item(name, value);
        ++size;
        return true;
    }

    bool removeItem(const std::string& name) {
        iterator_t new_end = std::remove_if(items_.begin(), items_.end(), Item::MatchesName(name));
        size_t num_elems_removed = std::distance(new_end, items_.end());
        size -= num_elems_removed;
        return num_elems_removed > 0;
    }

    Item* findItem(const std::string& name) {
        iterator_t found = std::find_if(items_.begin(), items_.end(), Item::MatchesName(name));
        if (found == items_.end())
            return nullptr;
        return &(*found);
    }
    ...
    void display() const {
        ...
        for (size_t i = 0u; i < size; ++i) {
            std::cout << "   " << i << ". " << std::left << std::setw(20) << items_[i].getName()
                      << " - " << items_[i].getValue() << " gold\n";
        }
    }

    int getCount() const { return size; }
    constexpr size_t getCapacity() const { return Cap; }
};
...
class QuestManager {
   ...
   public:
    void addQuest(const Quest& quest) { quests_[quest.id] = quest; }
    ...
    bool isCompleted(const std::string& questId) const {
        return completedQuests_.find(questId) != completedQuests_.end();
    }
    ...
    std::vector<Quest> getCompletedQuests() const {
        ...
        for (const auto& pair : quests_) {
            if (pair.second.completed)
                done.push_back(pair.second);
        }
        return done;
    }

    size_t getQuestCount() const { return quests_.size(); }
    size_t getCompletedCount() const { return completedQuests_.size(); }
};
...
inline void sortByHealth(std::vector<PlayerRecord>& players) {
    std::sort(players.begin(), players.end(),
              [](const PlayerRecord& a, const PlayerRecord& b) { return a.health < b.health; });
}

inline PlayerRecord* findByName(std::vector<PlayerRecord>& players, const std::string& name) {
    auto it = std::find_if(players.begin(), players.end(),
                           [&name](const PlayerRecord& p) { return p.name == name; });
    return it != players.end() ? &(*it) : nullptr;
}

inline void removeDead(std::vector<PlayerRecord>& players) {
    players.erase(std::remove_if(players.begin(), players.end(),
                                 [](const PlayerRecord& p) { return p.health <= 0; }),
                  players.end());
}

inline int countWithItem(const std::vector<PlayerRecord>& players, const std::string& item) {
    return static_cast<int>(
        std::count_if(players.begin(), players.end(),
                      [&item](const PlayerRecord& p) { return p.hasItem(item); }));
}
...
inline bool anyHasItem(const std::vector<PlayerRecord>& players, const std::string& item) {
...
class GameEngine {
   private:
    ...
    std::unique_ptr<Inventory<20>> inventory_;
   ...
   private:
    ...
    void describeLocation() {
        ...
        // SortedList: alphabetical treasure names (ties resolved by matching values below)
        if (!loc.treasureNames.empty()) {
            std::cout << "\n✨ You see treasure here:\n";
            SortedList<std::string> sortedNames;
            for (const auto& n : loc.treasureNames)
                sortedNames.insert(n);
            if (static_cast<size_t>(sortedNames.size()) == loc.treasureNames.size()) {
                for (auto it = sortedNames.begin(); it != sortedNames.end(); ++it) {
                    for (size_t i = 0; i < loc.treasureNames.size(); ++i) {
                        if (loc.treasureNames[i] == *it) {
                            std::cout << "   - " << *it << " (" << loc.treasureValues[i]
                                      << " gold)\n";
                            break;
                        }
                    }
                ...
                for (size_t i = 0; i < loc.treasureNames.size(); ++i) {
                    std::cout << "   - " << loc.treasureNames[i] << " (" << loc.treasureValues[i]
                              << " gold)\n";
                }
            }
        }
    ...
    void fight() {
        ...
        std::cout << "\n⚔️  COMBAT: You vs " << enemy->getName() << " (" << enemy->getType()
                  << ")\n\n";
        ...
        while (enemy->isAlive() && playerHealth_ > 0) {
            ...
            std::cout << "You attack for " << damage << " damage! (" << enemy->getName()
                      << " HP: " << enemy->getHealth() << ")\n";
    ...
    void loot() {
        ...
        priority_stack<std::string, int> presentation;
        for (size_t i = 0; i < loc.treasureNames.size(); ++i) {
            ...
            if (tname == "Magic Amulet")
                checkQuestCompletion("treasure");

            int prio = 1;
            if (tname.find("Amulet") != std::string::npos)
                prio = 3;
            else if (tname.find("Sword") != std::string::npos ||
                     tname.find("Dagger") != std::string::npos)
                prio = 2;

            presentation.push(prio, "   - " + tname + " (" + std::to_string(value) + " gold)");

            if (tname.find("Sword") != std::string::npos ||
                tname.find("Dagger") != std::string::npos) {
                int weaponDamage = value / 10;
                equippedWeapon_ = std::make_unique<Weapon>(tname, weaponDamage);
                presentation.push(2, "   ⚔️  Equipped " + tname + " (+" +
                                         std::to_string(weaponDamage) + " damage)");
            }
        }
        ...
        if (presentation.size() > 0) {
            ...
            for (size_t i = 0; i < loc.treasureNames.size(); ++i) {
                const std::string& tname = loc.treasureNames[i];
                int value = loc.treasureValues[i];
                std::cout << "   - " << tname << " (" << value << " gold)\n";
                if (tname.find("Sword") != std::string::npos ||
                    tname.find("Dagger") != std::string::npos) {
                    int weaponDamage = value / 10;
                    std::cout << "   ⚔️  Equipped " << tname << " (+" << weaponDamage
                              << " damage)\n";
                }
            }
        }
    ...
    void showQuests() {
        ...
        if (!active.empty()) {
            std::cout << "\n🔸 Active Quests:\n";
            SortedList<std::string> sortedTitles;
            for (const auto& quest : active)
                sortedTitles.insert(quest.name);
            if (static_cast<size_t>(sortedTitles.size()) == active.size()) {
                for (auto it = sortedTitles.begin(); it != sortedTitles.end(); ++it)
                    std::cout << "   [ ] " << *it << "\n";
            } else {
                for (const auto& quest : active)
                    std::cout << "   [ ] " << quest.name << "\n";
        ...
        if (!completed.empty()) {
            std::cout << "\n✅ Completed Quests:\n";
            SortedList<std::string> sortedDone;
            for (const auto& quest : completed)
                sortedDone.insert(quest.name);
            if (static_cast<size_t>(sortedDone.size()) == completed.size()) {
                for (auto it = sortedDone.begin(); it != sortedDone.end(); ++it)
                    std::cout << "   [✓] " << *it << "\n";
            } else {
                for (const auto& quest : completed)
                    std::cout << "   [✓] " << quest.name << "\n";
